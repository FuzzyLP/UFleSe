...
import filters.CiaoAndCSVFilesFilterClass;
...
public class FilesManagerAux {
	...
	{
		try{
			if( sharingState.get(p.getFileOwner()) != null )
	...
	private static ArrayList<ProgramFileInfo> listProgramFilesInSubDir(String subDir, PathsMgmt pathsMgmt,
		...
		FilenameFilter filter = (FilenameFilter) new CiaoAndCSVFilesFilterClass();
	...
	public static String[] uploadFileAux(RequestStoreHouse requestStoreHouse, FilesManager filesM) throws FilesAndPathsException, RequestStoreHouseException,
			...
			if (!fileItem.isFormField()) {
				...
				if (fileName == null) {
					...
					if ("".equals(fileName)) {
						...
						if (!fileName.endsWith(".pl")&&!fileName.endsWith(".csv")&&!fileName.endsWith(".json")&&!fileName.endsWith(".xlsx")&&!fileName.endsWith(".xls")&&!fileName.endsWith(".sql")) {
							...
							if (programFileInfo.existsFile(false)) {
								...
								if (writeWasOk) {
									if (fileName.endsWith(".csv") || fileName.endsWith(".json") || fileName.endsWith(".xlsx")|| fileName.endsWith(".xls")|| fileName.endsWith(".sql"))
									{
									...
									finally
									{
										...
										else
										{
											...
											ret.add("The file uploaded could not be compiled.");
										}
